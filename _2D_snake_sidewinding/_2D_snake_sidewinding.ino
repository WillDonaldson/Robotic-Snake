...
void sidewind(String Direction){
  ...
  for(int i=0; i<360; i++){
   ...
   for(int j=0; j<5; j++){  
      myServos[2*j].write(90+offset+amplitude*sin(Speed*rads+Multiplier*j*Wavelengths*shift));   //moves servos in vertical plane
      myServos[2*j+1].write(90+offset+amplitude*sin(Speed*rads+Multiplier*j*Wavelengths*shift)); //moves servos in horizontal plane
     }
   delay(10);
  }
}
void sidewindTurn(String Direction){
  ...
  for(int i=0; i<360; i++){
   rads=i*pi/180.0;     //convert from degrees to radians
   for(int j=0; j<3; j++){  
     myServos[2*j].write(90+offset+Multiplier*amplitude*sin(Speed*rads+j*Wavelengths*shift));
     myServos[2*j+1].write(90+offset+Multiplier*amplitude*sin(Speed*rads+j*Wavelengths*shift));
   }
   for(int j=3; j<5; j++){  
     myServos[2*j].write(90+offset-Multiplier*amplitude*sin(Speed*rads+j*Wavelengths*shift));
     myServos[2*j+1].write(90+offset-Multiplier*amplitude*sin(Speed*rads+j*Wavelengths*shift));
